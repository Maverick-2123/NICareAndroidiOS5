...
class VideoScreen extends StatefulWidget {
  ...
  const VideoScreen(this.vidName, this.vidId, this.videoTitle, {Key? key}) : super(key: key);
...
class VideoScreenState extends State<VideoScreen> with TickerProviderStateMixin {
  ...
  Future<void> _fetchLikedVideos() async {
    var url = 'http://15.207.244.117:8080/api/liked_videos/${TeamPageState.username}/';
    try {
  ...
  Future<void> _fetchComments() async {
    var url = 'http://15.207.244.117:8080/api/fetch_comments/?video_id=${widget.vidId}';
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        ...
        if (jsonData != null && jsonData["data"] != null) {
          for (var comment in jsonData["data"]) {
            String displayComment = "${comment['posted_by']}: ${comment['content']}";
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ...
          : Stack(
        children: [
          // Video Player with Tap & Double-Tap Gestures
          GestureDetector(
          ...
          Container(
            ...
            child: widget.comments.isEmpty
              ...
              itemBuilder: (context, index) {
                ...
                final commentText = parts.sublist(1).join(': '); // Handle colons in comment text
